...
Therefore, as the simulation results under the training mode with only single dataset shuffle are apparent lower than that under the usual training mode with dataset shuffle at every epoch, it is highly recommended to perform the VBALA with cubic VDF and VBALU with SIF under the ***usual training mode with dataset shuffle at every epoch*** to further effectively compare with your research or explore possible improvements.
