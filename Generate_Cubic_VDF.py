...
def vehicle_distribution(VL,VW,VH,VR,Vcenter_ori):
    VBALA_fea = np.zeros([x_max - x_min + 1, y_max - y_min + 1, 6,7])
    VBALA_fea_num=np.zeros([ x_max - x_min + 1,y_max - y_min + 1, 6])
    ...
    if X_index >= x_min and X_index <= x_max:
        if Y_index >= y_min and Y_index <= y_max:
            ...
            grid_coie=(Vcenter[0]-grid_coor[0])/W_grid,(Vcenter[1]-grid_coor[1])/L_grid,(VH/2-Z_index*H_grid)/H_grid
            VBALA_fea[X_spatial,Y_spatial,Z_index,:]=np.array([grid_coie[0],grid_coie[1],grid_coie[2],VW/W_max,VL/L_max,VH/H_max,VR_t/2/np.pi])#Normalized sizes and locations coordinates. The azimuths in the paper is also normalized by the factor 2pi.
            VBALA_fea_num[X_spatial,Y_spatial,Z_index]=1
...
VBALA_Scene_all=np.zeros([int(np.sum(samplenum_each_initialization[init_start:init_end])),x_max-x_min+1,y_max-y_min+1,6,7])
...
for init_index in enumerate(range(init_start, init_end)):
    ...
    for scene_index in range(MS_locations.shape[0]):
        ...
        vehicle_weight_all = np.zeros([x_max - x_min + 1, y_max - y_min + 1, 6,7])
        vehicle_weight_num_all=np.zeros([x_max - x_min + 1, y_max - y_min + 1, 6])
        vehicle_num_sta = 0
        for vehicle_piece in vehicles_dic_dect:
            if vehicle_piece['extent'][2] * 2 > 1.6: #We filter out the vehicles whose heights are lower than the MS's antenna height.
        ...
        #Calculate the average of the parameters of all vehicles in each grid
