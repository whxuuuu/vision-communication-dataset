...
def vehicle_distribution(VL,VW,VH,VR,Vcenter_ori):
    VBALA_fea = np.zeros([y_max - y_min + 1, x_max - x_min + 1, 6])
    VBALA_fea_num=np.zeros([y_max - y_min + 1, x_max - x_min + 1])
    ...
    if X_index >= x_min and X_index <= x_max:
        if Y_index >= y_min and Y_index <= y_max:
            ...
            VBALA_fea[Y_spatial,X_spatial,:]=np.array([VW/W_max,VL/L_max,VH/H_max,VR_t/2/np.pi,grid_coie[0],grid_coie[1]])
            VBALA_fea_num[Y_spatial,X_spatial]=1
...
VBALA_Scene_all=np.zeros([int(np.sum(samplenum_each_initialization[init_start:init_end])),y_max-y_min+1,x_max-x_min+1,6])
...
for init_index in enumerate(range(init_start, init_end)):
    ...
    for scene_index in range(MS_locations.shape[0]):
        ...
        for vehicle_piece in vehicles_dic_dect:
            if vehicle_piece['extent'][2] * 2 > 1.6663856033942093:#1.547088+0.05+(3-1.547088-0.05)*(1.788678/2)/(19-1.788678/2)=1.6663856033942093 is the minimum height that can block the MS antenna in the BS coverage area
        ...
        for ii in range(vehicle_weight_all.shape[0]):
            for jj in range(vehicle_weight_all.shape[1]):
                    if vehicle_weight_num_all[ii,jj]!=0:
                        vehicle_weight_all[ii,jj,:]=vehicle_weight_all[ii,jj,:]/vehicle_weight_num_all[ii,jj]
...
np.save('.//VDF_Correct.npy',VBALA_Scene_all)
